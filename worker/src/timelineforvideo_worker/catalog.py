...
def catalog_path(output_root: Path) -> Path:
    return output_root / ".timelineforvideo" / "catalog.jsonl"


def catalog_paths(output_root: Path) -> list[Path]:
    primary = catalog_path(output_root)
    legacy = output_root / ".video2timeline" / "catalog.jsonl"
    paths = [primary]
    if legacy != primary:
        paths.append(legacy)
    return paths
...
def load_catalog(output_root: Path) -> dict[str, dict[str, Any]]:
    ...
    for path in catalog_paths(output_root):
        if not path.exists():
            continue
